...
if(is_array($GLOBALS['arrSecondFilter']))
	${$FILTER_NAME}=array_merge(${$FILTER_NAME},$GLOBALS['arrSecondFilter']);
...
$pageURL = $uri->GetUri();
$paramsToDelete = array("set_filter", "del_filter", "ajax", "bxajaxid", "AJAX_CALL", "mode", 'category');
foreach($arResult["ITEMS"] as $PID => $arItem)
{
	foreach($arItem["VALUES"] as $key => $ar)
	{
